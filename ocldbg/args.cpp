...
bool parse_flag_opts(ProgramArgs &args, std::string_view arg, int &i, int argc, char **argv) {
    if (parse_vector_opt(arg, "-O", "--one-line-before-file", i, argc, argv,
        ...
        parse_string_opt(arg, "", "--read-memory", i, argc, argv, args.read_memory)) {
        return true;
    }
...
void print_help() {
    std::cout << std::format(
        ...
        "  --read-memory <a>[:<n>]  Read n bytes (default 16) of accelerator global memory at hex "
        "address a at each stop (accelerator backend)\n"
